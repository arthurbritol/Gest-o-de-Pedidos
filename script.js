...
const localStoragePedidoMotoristaKey = "pedidoMotoristaDestino"; // Nova chave para motorista destino
...
// Novos filtros para a tela de carregamento
let globalFiltroCarregamento = "todos"; // 'todos' ou 'meus'
let globalFiltroMotoristaDestino = null; // Usado para filtrar pedidos destinados a um motorista específico
...
// Funções para motorista destino
...
function setPedidoMotoristaDestino(id, motorista) {
    ...
    const pedidoIndex = pedidos.findIndex(p => p.id === id);
    if (pedidoIndex !== -1) {
        pedidos[pedidoIndex].motorista = motorista; // Atualiza o motorista no array de pedidos
    }
    renderizarPedidosCarregamento();
}
...
function showUserSelectionPage() {
    ...
    document.getElementById("loaderOrdersScreen").style.display = "none"; // Esconder tela de carregamento
...
function logout() {
    ...
    globalFiltroCarregamento = "todos"; // Resetar filtro de carregamento
    globalFiltroMotoristaDestino = null; // Resetar motorista destino
...
function resetAllOrderData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key.startsWith(localStorageDadosPesoKey) ||
            ...
            key.startsWith(localStoragePedidoMotoristaKey)) { // Adicionado a nova chave
    ...
    pedidos.forEach(p => {
        ...
        // Resetar motorista destino para o valor original do pedido
